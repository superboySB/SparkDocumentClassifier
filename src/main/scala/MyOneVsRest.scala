...
object MyOneVsRest{
  ...
  /**
    ...
    */
  trait OneVsRestParams extends PredictorParams
  ...
  object OneVsRestParams extends ClassifierTypeTrait {
  ...
  class OneVsRestModel private[ml] (
    ...
    override def transform(dataset: Dataset[_]): DataFrame = {
      ...
      val labelUDF1 = udf { (predictions: Map[Int, Double]) =>
//        println("Predict label: "+predictions.maxBy(_._2)._1.toDouble+" Probability: "+predictions.maxBy(_._2)._2)
//        predictions.maxBy(_._2)._2
        val sortList1=predictions.toList.sortBy(-_._2)
        sortList1(0)._2
      }

      val labelUDF2 = udf { (predictions: Map[Int, Double]) =>
        val sortList2=predictions.toList.sortBy(-_._2)
        sortList2(1)._2
      }

      val labelUDF3 = udf { (predictions: Map[Int, Double]) =>
        val sortList3=predictions.toList.sortBy(_._2)
        sortList3(1)._2
      }

      val labelUDF4 = udf { (predictions: Map[Int, Double]) =>
        val sortList4=predictions.toList.sortBy(_._2)
        sortList4(0)._2
      }
      ...
      aggregatedDataset
        ...
        .withColumn("Vote1",labelUDF1(col(accColName)),labelMetadata)
        .withColumn("Vote2",labelUDF2(col(accColName)),labelMetadata)
        .withColumn("Vote3",labelUDF3(col(accColName)),labelMetadata)
        .withColumn("Vote4",labelUDF4(col(accColName)),labelMetadata)
        .drop(accColName)
    }
  ...
  class OneVsRest (
    ...
    override def fit(dataset: Dataset[_]): OneVsRestModel = {
      ...
      // 这里的算法想法比较简单,是取决于label的最大值的,一定要注意！！！
